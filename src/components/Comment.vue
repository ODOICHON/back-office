        ...
        <div class="card bg-light">
            <div class="card-body">
                ...
                <form class="mb-4" @submit="handleSubmit">
                    ...
                    <textarea class="form-control" rows="3" id="commentContent" placeholder="로그인 후, 댓글을 남겨보세요!"
                ...
                <div class="mb-4" v-for="comment in comments" :key="comment.comment_id">
                    ...
                    <div v-if="comment.level === 1">
                        <div class="ms-3">
                            <div class="fw-bold">{{ comment.create_at }} | {{ comment.nick_name }}</div>
                            {{ comment.content }}
                            <!-- 답글 컴포넌트 -->
                            <div class="d-flex mt-4" v-for="reply in filteredReplies" :key="reply.level">
                                <div class="ms-3">
                                    <div class="fw-bold">{{ reply.create_at }} | {{ reply.nick_name }}</div>
                                    {{ reply.content }}
                                </div>
                            </div>
                            <!-- 답글 작성 폼 -->
                            <form class="mb-4" @submit="handleReplySubmit(comment.comment_id)">
                                ...
                                <textarea class="form-control" rows="3" id="replyContent" placeholder="답글을 남겨주세요."
...
export default {
    ...
    computed: {
        filteredReplies() : CommentType[] {
        return this.comments.filter(reply => reply.level >= 2);
        }
    },
    setup(prop) {
