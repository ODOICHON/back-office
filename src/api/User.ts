...
import { LoginReq, UserInfoResponse, UserResponse } from "@/types/UserType";
...
// 사용자 정보 조회
export function getUserInfo(headers : { [key: string]: string }) : AxiosPromise<UserInfoResponse> {
    return restApi.get(`${API_URLS.user}`, {headers});
}